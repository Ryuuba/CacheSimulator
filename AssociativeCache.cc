...
AssociativeCache::AssociativeCache(unsigned log2_capacity, unsigned log2_slotSize, Policy policy) :
Cache(std::pow(2, log2_slotSize), std::pow(2, log2_capacity)),
...
{           
    ...
    switch(policy) {
        case Policy::LFU:
            agent = new LFUPolicyAgent();
            break;
        case Policy::LRU:
            agent = new LRUPolicyAgent(policy, capacity);
            break;
        case Policy::FIFO:
            agent = new FIFOPolicyAgent();
            break;
        default:
            agent = new RandomPolicyAgent();
    }
}
